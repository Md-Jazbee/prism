...
- [ ] ACC-1 Doc-QA answerability ≥80% from one pack (live-judged) — **OPEN** (proxy 96% ≥1-span / 68% all-spans — see `eval/baselines/doc-qa-answerability/latest.json`)
...
- [x] ACC-4 Top-10 hubs contain 0 language builtins; Louvain communities shipped (label dual-review worksheet open — `eval/labeling/community-labels-p12-sample.json`)
...
**Code complete for Stages A–C** (docs layer, honest gaps, ACC-3 grounding, Louvain+hubs+bridges, G4 doc-edit incremental test, planted-secret fixture). Live packs: capped doc prose + README thesis; Louvain activates via co-directory edges. Stage D harness + ACC-1 proxy + label worksheet archived. **Phase gate remains OPEN** until live-judged ACC-1/ACC-5/ACC-7.
